...
(function () {
  ...
  function drawCombinedBarChart(target, helperData, helpeeData, options) {
    ...
    const totalColWidth = options.totalColWidth != null ? options.totalColWidth : 180;
    ...
    const helperHeaderX = totalColX + totalColWidth * (1 / 6);
    const helpeeHeaderX = totalColX + totalColWidth * (3 / 6);
    const sumHeaderX = totalColX + totalColWidth * (5 / 6);
    ctx.fillStyle = '#2563eb';
    ctx.fillText('応援', helperHeaderX, padding.top - 4);
    ctx.fillStyle = '#dc2626';
    ctx.fillText('依頼', helpeeHeaderX, padding.top - 4);
    ctx.fillStyle = '#475569';
    ctx.fillText('合計', sumHeaderX, padding.top - 4);
    ...
    for (let i = 0; i < entries.length; i++) {
      ...
      ctx.fillStyle = '#0f172a';
      ctx.fillText(String(e.helperCount + e.helpeeCount), sumHeaderX, groupCenter);
    }
